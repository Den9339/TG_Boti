...
class Database:
    ...
    def format_args(sql, parameters: dict) -> tuple:
        ...
        ### Команды для таблицы с товарами
    ...
    def add_item(self, id: int, name: str, quantity: int = None):
        sql = 'INSERT INTO Items(id, name, quantity) VALUES(?, ?, ?)'
        parameters = (id, name, quantity)
        self.execute(sql, parameters, commit=True)
    ...
    def update_items_info(self, id: int, name: str, quantity: int = None):
        sql = 'UPDATE Items SET name=? AND quantity=? WHERE id=?'
